...
loss = f.read().split('\n')
f.close()
f = open('loss_without_offset.txt')
...
plt.plot(x_range, without_offset_train_datas, label='loss_train without offset')
plt.plot(x_range, without_offset_test_datas, label='loss_test without offset')
