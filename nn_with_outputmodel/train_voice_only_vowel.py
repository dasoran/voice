...
# voice model
vm_freq = 180 # 100 - 200 from wikipedia
t = 0
bitrate = 48000
def initalize_model(frequency):
    vm_freq = frequency

def make_output(fn_1, cn_1, fn_2, cn_2, fn_3, cn_3):
    if cn_1 < 0.1 and cn_2 < 0.1 and cn_3 < 0.1:
        return 0
    fn_1 = math.floor(fn_1 * 50)
    fn_2 = math.floor(fn_2 * 50)
    fn_3 = math.floor(fn_3 * 50)
    outsource_1 = sin(2 * math.pi * (t / bitrate) * vm_freq * fn_1)
    outsource_2 = sin(2 * math.pi * (t / bitrate) * vm_freq * fn_2)
    outsource_3 = sin(2 * math.pi * (t / bitrate) * vm_freq * fn_3)
    out = cn_1 * outsource_1 + cn_2 * outsource_2 + cn_3 * outsource_3
    t = t + 1
    return out
...
setattr(model, 'l' + str(len(n_unitslist) + n_add_layer + 1) + '1f'
        , F.Linear(500, 1000))
...
def network(x, train):
    ...
    y1f = F.tanh(getattr(model, 'l' + str(len(n_unitslist) + n_add_layer + 1) + '1f')(acc))
    return out
